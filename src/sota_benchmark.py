#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Run supplemental baseline benchmarks for the DART stock prediction repo.
...
def write_markdown_summary(rows, args, path):
    ...
    lines = [
        "# Supplemental Baseline Benchmark",
        "",
        ...
        "## Notes",
        "",
        "- `CausalStock` augments price features with daily news sentiment features and a sparse causal graph.",
        "- `MambaStock` is a lightweight selective state-space baseline that uses price features only.",
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run supplemental baselines, including CausalStock and MambaStock."
    )
